...
const Item = ({productData=productInfo[0] ,maxQuantity=10, minQuantity=1}) => {
  ...
  return (
    <div id='itemContainer' className="w-fit mx-auto rounded-lg p-4 max-sm:px-0 max-sm:border-none">
       <div className="block gap-4 md:flex-row">
         {/* Item options menu */}
         <div className="w-full z-40 p-4 h-fit mx-auto self-center float-left md:w-1/3  bg-gray-50 rounded-lg shadow-xl border">
          <Typography variant="h6" className="mb-2"> מחיר יחידה: {currDict[productData.currency]}{productData.price}</Typography>
          {productData.discount && <Typography variant="body2" className="text-secondary mb-4">הנחה: {Math.floor(100*(1-productData.price/productData.originalPrice))}%</Typography>}
            {/*    Select from produt's buying Options   */}
            {Object.keys(productData.buyingOptions).map((item,index)=>(
                <div key={index} className='my-4'>
                <Typography  variant="body1" className="mb-2">{`בחירת ${item}`}</Typography>
                    <Select
          ...
          <Typography variant="body1" className="mb-2">כמות: </Typography>
          ...
          <Typography variant="h6" className="mb-2">מחיר כולל:  </Typography>
          <Typography variant="h6" className="mb-2">{currDict[productData.currency]}{Number(quantity*productData.price).toFixed(2)}</Typography>
            </div>
        ...
        <div className="w-full p-4 max-sm:py-8 max-xs:px-1">
            ...
            {productData.video ?
                <div className="my-6 py-4 mx-auto max-sm:px-2 overflow-hidden w-fit h-fit">
                <iframe 
                className='rounded-lg shadow-lg'
                width={width<400 ? width-20 :2*width/3} 
