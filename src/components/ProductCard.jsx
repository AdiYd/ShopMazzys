...
import Item, { currDict } from './Item';
...
const SaleIcon = (...props) => (
   <svg
    ...
    viewBox="0 0 24 24"
    className={`w-9 text-green-700 -rotate-6 h-16 absolute opacity-80 -right-12 -bottom-2`}
    fill="currentColor"
  >
...
const ProductCard = ({ 
  ...
  return (
    ...
    className={`card mx-auto ${border ? 'border':''} ${shadow ? 'shadow-md':''} cursor-pointer ${containerClass}`} style={{height:cardHeight}}>
      ...
      id='cardImg' 
      src={imageSrc} 
      style={{width: width<250 ? `${width/2-10}px`: `${imageWidth}px`, height:imageWidth&&!imageClass ?`${0.8*imageWidth}px`:undefined }}
      ...
      {showPrice && <div onClick={onClick} className="mt-2 mx-auto flex justify-center">
        ...
        <p className="font-bold text-base text-red-800 mr-2">{currDict[productData.currency]}{productData.price}</p>
      </div>}
        {(showProductCount && showButton )?  
              <div dir='ltr' className="flex w-auto mx-4 align-baseline max-xs:mx-1 justify-between max-xs:flex-col-reverse max-xs:gap-2 items-center mt-2">
